...
class Model:
    def __init__(self, model, parameters, constraints=None):
        ...
        for parameter in parameters:
            ...
            setattr(self.__class__, parameter.name, property(self.__gitem(parameter.name), self.__sitem(parameter.name)))
    ...
    def func(self):
        def inner(x):
            return self._fun(x, *[self._parameters[key].value for key in self._parameters.keys()])
        return inner
    ...
    @property
    def x0(self):
        return [self._parameters[key].value for key in self._parameters.keys() if not self._parameters[key].fixed]

    def set_parameters(self, *args):
        items = [self._parameters[key] for key in self._parameters.keys() if not self._parameters[key].fixed]
        if len(args) == 1 and isinstance(args[0], list):
            args = args[0]
        if len(items) != len(args):
            raise ValueError(f'Incorrect number of parameters. Expected {len(items)}, got {len(args)}.')
    ...
    @staticmethod
    def __sitem(key):
        return lambda obj, value: obj._parameters.__getitem__(key).__setitem__('value', value)
